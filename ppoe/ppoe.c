...
void print_error(char** result,            /*location whhere string is located, must be freed*/
  ...
  if (error != error) {   /* validate and figure the precision of error out */
...
appender formatter<std::pair<double, double>>::format(std::pair<double, double>& value_error, format_context& context) const {
  ...
  char conversion; /* p, q, r while simelar to p~e, q~f, r~g and P, Q, R */
  if (specs_.type == presentation_type::exp_lower) conversion = 'p';
  if (specs_.type == presentation_type::exp_upper) conversion = 'P';
  if (specs_.type == presentation_type::fixed_lower) conversion = 'q';
  if (specs_.type == presentation_type::fixed_upper) conversion = 'Q';
  if (specs_.type == presentation_type::general_lower) conversion = 'r';
  if (specs_.type == presentation_type::general_upper) conversion = 'R';

  char padding_character = specs_.fill[0];  // no UTF

  short side_adjustment = -1; /* supported */
  if (specs_.align == align::type::left ) side_adjustment = 1;
  if (specs_.align == align::type::center) side_adjustment = 0;
  if (specs_.align == align::type::right) side_adjustment = -1;

  bool flag_sign = false; /* supported */
  char flag_startwithblank = '\0';
  if (specs_.sign == sign::type::plus) flag_sign = true;
  if (specs_.sign == sign::type::minus || specs_.sign == sign::type::space) flag_sign = false;
  if (specs_.sign == sign::type::minus) {
    flag_sign = false;
    flag_startwithblank = ' ';
  }
